class Medico:
    ...
    def adicionar_horario(self, horario: str):
        if horario in self.agenda:
            raise ValueError("Este horário já está reservado na agenda do médico.")
        self.agenda.append(horario)

    def remover_horario(self, horario: str):
        if horario not in self.agenda:
            raise ValueError("Não é possível remover um horário que não está registrado na agenda.")
